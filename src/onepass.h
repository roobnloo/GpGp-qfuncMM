...
arma::mat backward_solve_mat(arma::mat cholmat, arma::mat b) {
  ...
  arma::mat x(n, p);
  for (int k = 0; k < p; k++) {
    x(n - 1, k) = b(n - 1, k) / cholmat(n - 1, n - 1);
  }
...
void compute_pieces(arma::vec covparms, StringVector covfun_name,
                    arma::mat locs, arma::mat NNarray, arma::vec y, arma::mat X,
                    mat *XSX, vec *ySX, double *ySy, double *logdet, cube *dXSX,
                    mat *dySX, vec *dySy, vec *dlogdet, mat *ainfo,
                    int profbeta, int grad_info,
  ...
  /* p_covfun is an array of length 1. Its entry is a pointer to a function
   which takes in arma::vec and arma::mat and returns mat. p_d_covfun is
   analogous. This was a workaround for the solaris bug*/
...
#pragma omp for
    for (int i = 0; i < n; i++) {
      ...
      for (int j = bsize - 1; j >= 0; j--) {
        ...
        for (int k = 0; k < dim; k++) {
          locsub(bsize - 1 - j, k) = locs(NNarray(i, j) - 1, k);
        }
        if (profbeta) {
          for (int k = 0; k < p; k++) {
            X0(bsize - 1 - j, k) = X(NNarray(i, j) - 1, k);
          }
        }
      }
      ...
      if (grad_info) {
        ...
        if (cond) { // if we condition on anything
          ...
          for (int j = 0; j < nparms; j++) {
            ...
            // arma::vec LidSLi3 = solve( trimatl(cholmat), dcovmat.slice(j) *
            // choli2 );
            arma::vec LidSLi3 =
                forward_solve(cholmat, dcovmat.slice(j) * choli2);
            // store LiX0.t() * LidSLi3 and Liy0.t() * LidSLi3
            arma::vec v1 = LiX0.t() * LidSLi3;
            double s1 = as_scalar(Liy0.t() * LidSLi3);
            // update all quantities
            // bottom-right corner gets double counted, so need to subtract it
            // off
            (l_dXSX).slice(j) +=
            ...
            (l_dySX).col(j) +=
                (s1 * LiX0.rows(i2) + (v1 * Liy0(i2)).t() -
                 as_scalar(LidSLi3(i2)) * LiX0.rows(i2) * as_scalar(Liy0(i2)))
                    .t();
          ...
          for (int i = 0; i < nparms; i++) {
            for (int j = 0; j < i + 1; j++) {
              (l_ainfo)(i, j) +=
                  ...
                  0.5 * accu(LidSLi2.rows(i2).col(j) % LidSLi2.rows(i2).col(i));
            }
          }

        } else { // similar calculations, but for when there is no conditioning
                 // set
          for (int j = 0; j < nparms; j++) {
      ...
      //         cout <<
      //         std::chrono::duration_cast<std::chrono::microseconds>(tt[k]-tt[k-1]).count();
      //         cout << "  ";
      //     }
      //     cout << endl;
      // }
...
void compute_pieces_grouped(arma::vec covparms, StringVector covfun_name,
                            arma::mat locs, List NNlist, arma::vec y,
                            arma::mat X, mat *XSX, vec *ySX, double *ySy,
                            double *logdet, cube *dXSX, mat *dySX, vec *dySy,
                            vec *dlogdet, mat *ainfo, bool profbeta,
                            bool grad_info, const arma::mat additional_info) {
...
#pragma omp for
    for (int i = 0; i < nb; i++) {
      ...
      if (i == 0) {
        ...
        first_ind = last_ind_of_block[i - 1] + 1 - 1;
      }
      ...
      if (i == 0) {
        ...
        first_resp = last_resp_of_block[i - 1] + 1 - 1;
      }
      ...
      if (grad_info) {
        ...
        if (cond) { // if we condition on anything
          ...
          for (int j = 0; j < nparms; j++) {
            // compute last column of Li * (dS_j) * Lit
            // arma::mat LidSLi4 = solve( trimatl(cholmat), dcovmat.slice(j) *
            // choli2 );
            arma::mat LidSLi4 =
                forward_solve_mat(cholmat, dcovmat.slice(j) * choli2);
            ...
            for (int k = 0; k < rsize; k++) {
              ...
              // off
              (l_dXSX).slice(j) +=
                  v1 * LiX0.row(i2) + (v1 * LiX0.row(i2)).t() -
                  LidSLi3(i2) * LiX0.row(i2).t() * LiX0.row(i2);
              (l_dySy)(j) +=
                  2.0 * s1 * Liy0(i2) - LidSLi3(i2) * Liy0(i2) * Liy0(i2);
              (l_dySX).col(j) += (s1 * LiX0.row(i2) + (v1 * Liy0(i2)).t() -
                                  LidSLi3(i2) * LiX0.row(i2) * Liy0(i2))
                                     .t();
          ...
          for (int i = 0; i < nparms; i++) {
            for (int j = 0; j < i + 1; j++) {
              (l_ainfo)(i, j) +=
                  0.5 * accu(LidSLi2.slice(i) % LidSLi2.slice(j));
            }
          }
        }
      }
...
void synthesize(NumericVector covparms, StringVector covfun_name,
                const NumericMatrix locs, NumericMatrix NNarray,
                NumericVector &y, NumericMatrix X, NumericVector *ll,
                NumericVector *betahat, NumericVector *grad,
                NumericMatrix *info, NumericMatrix *betainfo, bool profbeta,
                bool grad_info,
  ...
  arma::mat NNarray_c =
      arma::mat(NNarray.begin(), NNarray.nrow(), NNarray.ncol());
  arma::vec y_c = arma::vec(y.begin(), y.length());
  arma::mat X_c = arma::mat(X.begin(), X.nrow(), X.ncol());

  compute_pieces(covparms_c, covfun_name, locs_c, NNarray_c, y_c, X_c, &XSX,
                 &ySX, &ySy, &logdet, &dXSX, &dySX, &dySy, &dlogdet, &ainfo,
...
void synthesize_grouped(NumericVector covparms, StringVector covfun_name,
                        const NumericMatrix locs, List NNlist, NumericVector &y,
                        NumericMatrix X, NumericVector *ll,
                        NumericVector *betahat, NumericVector *grad,
                        NumericMatrix *info, NumericMatrix *betainfo,
  ...
  compute_pieces_grouped(covparms_c, covfun_name, locs_c, NNlist, y_c, X_c,
                         &XSX, &ySX, &ySy, &logdet, &dXSX, &dySX, &dySy,
                         &dlogdet, &ainfo, profbeta, grad_info,
                         additional_info_c);
...
// [[Rcpp::export]]
NumericMatrix
vecchia_Linv(arma::vec covparms, StringVector covfun_name, arma::mat locs,
             arma::mat NNarray, int start_ind = 1,
